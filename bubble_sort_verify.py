"""
VERIFICATION SUITE for bubble_sort_analyzer.py

Standalone. This program verifies bubble sort and nothing else.

USAGE
    Put this file in the SAME FOLDER as bubble_sort_analyzer.py, then:
        python bubble_sort_verify.py
...
try:
    from bubble_sort_analyzer import (
    ...
    sys.exit(f"Cannot import bubble_sort_analyzer.py: {exc}\n"
             "Put this file in the SAME FOLDER as bubble_sort_analyzer.py, and\n"
...
def run():
    ...
    print("\n[7] SWAPS == INVERSION COUNT (each swap removes exactly one)")
    print("     A bubble swap exchanges an ADJACENT out-of-order pair, which")
    print("     removes precisely one inversion. These must be EQUAL, not")
    print("     merely correlated.")
    print(f"       {'disorder':>9} {'inversions d':>13} {'swaps':>8}")
    ok_eq = True
    for n, frac in ((60, 0.0), (60, 0.1), (60, 0.5), (60, 1.0)):
        a = _scramble(n, frac)
        d = inversions(a)
        _, swaps = sort_counted(a.copy())
        ok_eq &= (swaps == d)
        print(f"       {frac:>9.0%} {d:>13} {swaps:>8}")
    check("swaps == d exactly, at every disorder level", ok_eq)

    print("\n[8] EARLY EXIT ACTUALLY FIRES (without it there is no O(n) best case)")
    n = 500
    c_sorted, _ = sort_counted(make_array(n, "best", SEED))
    c_worst, _ = sort_counted(make_array(n, "worst", SEED))
    check("sorted input costs n-1 comparisons, not n(n-1)/2",
          c_sorted == n - 1 and c_worst == n * (n - 1) // 2,
          f"{c_sorted} vs {c_worst:,}  ({c_worst / c_sorted:.0f}x cheaper)")
