"""
INSERTION SORT COMPLEXITY ANALYZER

Standalone. This program analyses insertion sort and nothing else.
...
Usage:  python insertion_sort_analyzer.py
Verify: python insertion_sort_verify.py
"""
...
MOVE_NAME = "shifts"          # what the second metric actually IS
MOVE_ABBR = "shf"
STABLE    = True
ADAPTIVE  = True

# Expected exponent k in T(n) ~ n^k, per case. Drives the pass/fail verdict.
EXPECTED = {"best": 1.0, "average": 2.0, "worst": 2.0}

CASE_LABEL = {
    "best":    "Best  O(n)  sorted",
    "average": "Average  O(n^2)  random",
    "worst":   "Worst  O(n^2)  reversed",
}


def sort(a):
    """
    Pure, uninstrumented insertion sort. This is the function that gets timed.

    It SHIFTS rather than swaps. Each inner step is one write, not a three-way
    exchange. That single fact is why insertion sort carries the smallest
    constant factor of the elementary quadratic sorts.
    """
...
def sort_counted(a):
    ...
    The short-circuit matters: in `while j >= 0 and a[j] > key` the comparison
    a[j] > key is NOT evaluated when j < 0, so it must not be counted there.
    Getting this wrong inflates the best case from n-1 to n.
    """
    n = len(a)
    comparisons = moves = 0
    for i in range(1, n):
...
REFERENCES = [
    ("n(n-1)/2  cmp+shift worst", lambda n: n * (n - 1) / 2, WARN, ":"),
    ("n-1  cmp best", lambda n: n - 1, "#9c6bff", "-."),
]
...
SUMMARY = (
"INSERTION SORT  ::  ANALYTICAL\n"
"----------------------------------------\n"
"TIME\n"
"  best     O(n)      already sorted\n"
"  average  O(n^2)    random order\n"
"  worst    O(n^2)    reverse sorted\n"
"  exact    O(n + d), d = inversions\n"
"\n"
"COMPARISONS\n"
"  best     n - 1\n"
"  worst    n(n-1)/2\n"
"\n"
"SHIFTS (1 write each, not swaps)\n"
"  best     0\n"
"  worst    n(n-1)/2\n"
...
"\n"
"Base case of Timsort, which is what\n"
"Python's own sorted() runs.\n"
)
...
DEFINITION
  Grows a sorted prefix one element at a time. It takes the next element,
  walks it leftward past every larger element in the prefix, and drops it into
  place. This is how nearly everyone sorts a hand of playing cards.

  In-place (O(1) auxiliary space) and STABLE, because the shift condition is
  strictly a[j] > key. An equal element stops the walk, so equal keys never
  cross each other.
...
HOW IT WORKS
  for i in 1 .. n-1:
      ...
      j = i - 1
      while j >= 0 and a[j] > key:
          a[j+1] = a[j]          <- SHIFT, one write. Not a swap.
          j -= 1
      a[j+1] = key

  Two things worth noticing.

  1. It SHIFTS, it does not swap. A swap is three writes (temp, a, b). A shift
     is one. Some other quadratic sorts perform the same asymptotic number of
     element moves but pay roughly 3x per move. This is the single biggest
     reason insertion sort is the fastest of the elementary sorts in practice.

  2. The `while` short-circuits on j >= 0 BEFORE evaluating a[j] > key, so the
     comparison is not performed when the walk falls off the left edge. Miss
     this when counting and the best case reads n instead of n-1.

COMPLEXITY
  ------------------------------------------------------------------
  case       time      comparisons    shifts       input
  ------------------------------------------------------------------
  best       O(n)      n - 1          0            already sorted
  average    O(n^2)    ~n^2/4         ~n^2/4       random
  worst      O(n^2)    n(n-1)/2       n(n-1)/2     reverse sorted
  ------------------------------------------------------------------
  Auxiliary space: O(1) in every case.

  In the best case each new element is compared once against the tail of the
  prefix, fails the > test immediately, and stays put. One comparison per
  element, zero shifts, n-1 comparisons in total.

ADAPTIVITY, THE REAL VIRTUE
  The exact running time is O(n + d), where d is the number of INVERSIONS in
  the input, meaning pairs that are out of order. Each shift removes exactly
  one inversion, so the shift count IS d.

  This is a genuinely strong property, not a technicality. Sorted input has
  d = 0. Reverse sorted has d = n(n-1)/2, the maximum. Real data, such as an
  append-mostly log or a nearly-sorted index, tends to have small d, and
  insertion sort runs in close to linear time on it.

  Timsort, which is Python's built-in sort, uses insertion sort as its base
  case for short runs precisely because of this. Every time you call sorted(),
  the code on this page is running underneath.

WHAT TO EXPECT FROM THIS TOOL
  The best case will collapse onto the x-axis in panel 01 and fit n^1.0 in
  panel 04, while average and worst fit n^2.0. That gap between the best case
  and the other two is adaptivity, measured rather than asserted.

  In panel 02, watch the shift curve on the best case sit at exactly zero
  while comparisons sit at exactly n-1. Then switch to worst and watch both
  land on the dotted n(n-1)/2 line together, because on reverse-sorted input
  every comparison is followed by a shift.
"""
...
class Analyzer:
    ...
    def export_csv(self):
        ...
        path = filedialog.asksaveasfilename(
            ...
            initialfile="insertion_sort_complexity.csv")
        if not path:
            return
