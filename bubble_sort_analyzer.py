"""
BUBBLE SORT COMPLEXITY ANALYZER

Standalone. This program analyses bubble sort and nothing else.
...
Usage:  python bubble_sort_analyzer.py
Verify: python bubble_sort_verify.py
"""
...
def sort(a):
    """
    Pure, uninstrumented bubble sort. This is the function that gets timed.

    Two details that are commonly got wrong:
      1. The inner bound is n-i-1, not n-1. After pass i the last i elements
         are already in final position; rescanning them is wasted work.
      2. The `swapped` flag with the early return is the ONLY source of the
         O(n) best case. Delete it and the best case degrades to O(n^2).
    """
    n = len(a)
    for i in range(n - 1):
        swapped = False
...
REFERENCES = [
    ("n(n-1)/2  cmp bound", lambda n: n * (n - 1) / 2, WARN, ":"),
]
...
"  best     0\n"
"  worst    n(n-1)/2\n"
"  swaps == inversion count\n"
"\n"
"SPACE\n"
"  auxiliary  O(1)   in-place\n"
"\n"
"adaptive: passes only   stable: yes\n"
"\n"
"Run to fit the empirical exponent\n"
...
DEFINITION
  Repeatedly steps through the list, compares each adjacent pair, and swaps
  them if they are out of order. Each pass "bubbles" the largest remaining
  element to the end of the unsorted region. The list is sorted when a pass
  completes with no swaps.

  In-place (no scratch space proportional to n) and STABLE, because the swap
  test is strictly a[j] > a[j+1], never >=. Equal elements never cross.
  ...
  for i in 0 .. n-2:
      ...
      for j in 0 .. n-i-2:
          if a[j] > a[j+1]:
              ...
              swapped = true
      if not swapped:
          break            <- the early exit. This is what makes it adaptive.

  Two details that are commonly got wrong:

  1. The inner bound is n-i-1, not n-1. After pass i the last i elements are
     already final, so rescanning them is wasted work.
  2. The `swapped` flag with the early break is the ONLY source of the O(n)
     best case. Remove it and the best case degrades to O(n^2).
...
COMPLEXITY
  ...
  average    O(n^2)    ~n(n-1)/2      ~n(n-1)/4    random
  worst      O(n^2)    n(n-1)/2       n(n-1)/2     reverse sorted
  ------------------------------------------------------------------
  Auxiliary space: O(1) in every case.

  The worst case is EXACTLY n(n-1)/2 comparisons because no early exit ever
  fires: every pass performs at least one swap. At n = 900 that is 404,550,
  the dotted reference line in panel 02.

SWAPS EQUAL INVERSIONS
  Every swap exchanges an ADJACENT out-of-order pair, and that removes exactly
  one inversion from the array. So the swap count is precisely the number of
  inversions in the input. Sorted input has zero. Reverse sorted has the
  maximum, n(n-1)/2.

ON THE "PREFERRED" BEST CASE
  O(n) sounds attractive but it is a narrow win: it only happens on input that
  is already sorted, which is the case where you did not need to sort at all.
  On NEARLY sorted data the early exit does help, but only by cutting the
  number of PASSES. Each surviving pass still rescans the whole unsorted
  region, so bubble sort cannot exploit near-sortedness as tightly as some
  other quadratic sorts can.

  Honest summary: assume O(n^2) and design around it. Bubble sort survives in
  curricula, not in production code. Its real value is that it is the clearest
  possible demonstration of quadratic growth, and this tool exists to measure
  that growth rather than take it on trust.

WHAT THIS PROGRAM PROVES
  It does not accept the O(n^2) label. It measures wall-clock time across a
  range of n, fits log T(n) = k * log n + c by least squares, and reports k.
  If the theory holds, k converges on 1 for the best case and 2 for the
  average and worst cases. That fitted k is the verdict in the RESULTS panel
  and in the title of panel 04.
"""
...
class Analyzer:
    ...
    def export_csv(self):
        ...
        path = filedialog.asksaveasfilename(
            ...
            initialfile="bubble_sort_complexity.csv")
        if not path:
            return
